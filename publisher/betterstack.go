...
func syncBetterstackData(data []byte, wt *git.Worktree, fs billy.Filesystem) (plumbing.Hash, error) {
	rgb, err := fs.Open(betterstackFile)
	...
	if err == nil {
		...
		slog.Info(betterstackFile, "up to date", upToDate)
	}

	if err = createFile(fs, betterstackFile, data); err != nil {
		return plumbing.ZeroHash, err
	}

	if _, err = wt.Add(betterstackFile); err != nil {
		return plumbing.ZeroHash, err
	}

	return createCommit(wt, "update betterstack data")
}
