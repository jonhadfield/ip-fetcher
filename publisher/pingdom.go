...
func syncPingdomData(data []byte, wt *git.Worktree, fs billy.Filesystem) (plumbing.Hash, error) {
	...
	if err == nil {
		...
		slog.Info(pingdomFile, "up to date", upToDate)
	}

	if err = createFile(fs, pingdomFile, data); err != nil {
		return plumbing.ZeroHash, err
	}

	if _, err = wt.Add(pingdomFile); err != nil {
		return plumbing.ZeroHash, err
	}

	return createCommit(wt, "update pingdom data")
}
