...
func syncUpdownData(data []byte, wt *git.Worktree, fs billy.Filesystem) (plumbing.Hash, error) {
	...
	if err == nil {
		...
		slog.Info(updownFile, "up to date", upToDate)
	}

	if err = createFile(fs, updownFile, data); err != nil {
		return plumbing.ZeroHash, err
	}

	if _, err = wt.Add(updownFile); err != nil {
		return plumbing.ZeroHash, err
	}

	return createCommit(wt, "update updown data")
}
