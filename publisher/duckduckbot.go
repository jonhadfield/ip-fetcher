...
func syncDuckduckbotData(data []byte, wt *git.Worktree, fs billy.Filesystem) (plumbing.Hash, error) {
	rgb, err := fs.Open(duckduckbotFile)
	...
	if err == nil {
		...
		slog.Info(duckduckbotFile, "up to date", upToDate)
	}

	if err = createFile(fs, duckduckbotFile, data); err != nil {
		return plumbing.ZeroHash, err
	}

	if _, err = wt.Add(duckduckbotFile); err != nil {
		return plumbing.ZeroHash, err
	}

	return createCommit(wt, "update duckduckbot data")
}
