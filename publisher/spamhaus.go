...
func syncSpamhausData(data []byte, wt *git.Worktree, fs billy.Filesystem) (plumbing.Hash, error) {
	...
	if err == nil {
		...
		slog.Info(spamhausFile, "up to date", upToDate)
	}

	if err = createFile(fs, spamhausFile, data); err != nil {
		return plumbing.ZeroHash, err
	}

	if _, err = wt.Add(spamhausFile); err != nil {
		return plumbing.ZeroHash, err
	}

	return createCommit(wt, "update spamhaus data")
}
