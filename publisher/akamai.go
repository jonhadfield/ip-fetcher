...
func syncAkamaiData(data []byte, wt *git.Worktree, fs billy.Filesystem) (plumbing.Hash, error) {
	...
	if err == nil {
		...
		slog.Info(akamaiFile, "up to date", upToDate)
	}

	if err = createFile(fs, akamaiFile, data); err != nil {
		return plumbing.ZeroHash, err
	}

	if _, err = wt.Add(akamaiFile); err != nil {
		return plumbing.ZeroHash, err
	}

	return createCommit(wt, "update akamai data")
}
