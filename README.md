...
| --- | --- |
| **HetrixTools** | Uptime monitoring probe addresses published as a plain IP list (also resolvable via `ips.hetrixtools.com`). Use when allowlisting HetrixTools checks through a firewall. |
...
[^qualys]: Built from prefixes announced by AS27385 via RIPE Stat, not from a Qualys-published scanner file. A match means the address is Qualys-owned; a miss does not rule out a Qualys scan from a platform SOC listed only in your account.
