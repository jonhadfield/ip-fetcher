...
func virustotalCmd() *cli.Command {
	const (
	...
	return &cli.Command{
		Name:      providerName,
		HelpName:  "- fetch VirusTotal prefixes",
		Usage:     "VirusTotal",
		UsageText: "ip-fetcher virustotal {--stdout | --path FILE}",
		...
		Action: func(c *cli.Context) error {
			...
			if os.Getenv("IP_FETCHER_MOCK_VIRUSTOTAL") == "true" {
				defer gock.Off()
				urlBase := virustotal.DownloadURL
				u, _ := url.Parse(urlBase)
				gock.New(urlBase).
					Get(u.Path).
					Reply(http.StatusOK).
					File("../../providers/virustotal/testdata/ip-addresses.txt")
